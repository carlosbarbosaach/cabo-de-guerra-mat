...
export default function PaginaEquipeDinamica() {
    ...
    const gerar = (nivelAtual: string, operacaoAtiva: string) => {
        ...
        if (op === '-') {
            ...
            res = maior - menor; txt = `${maior} - ${menor}`;
        } else if (op === '*') {
    ...
    const responder = (e: React.FormEvent) => {
        ...
        if (dados.status !== 'jogando' || isNaN(respostaUsuario) || !equipeId) return;
        
        const forca = (NIVEIS[dados.nivel as keyof typeof NIVEIS] || NIVEIS['1EF']).forca;

        if (respostaUsuario === conta.res) {
            if (dados.modo === 'cabo') {
                update(ref(db, 'partida'), { posicao: increment(ehAzul ? -forca : forca) });
            } else {
                // CORREÇÃO DO ERRO .sv: Entramos direto na pasta da equipe
                set(ref(db, `partida/progressoEquipes/${equipeId}`), increment(forca));
            }
        ...
        <div className={`h-screen flex flex-col items-center justify-center p-6 text-white transition-all duration-500 ${dados.status === 'jogando' ? (ehAzul ? 'bg-[#0542b9]' : 'bg-[#c60929]') : 'bg-[#46178f]'}`}>
            <div className="bg-white text-slate-900 p-8 rounded-[2.5rem] shadow-2xl w-full max-w-lg text-center border-b-[10px] border-black/10">
                {dados.status === 'jogando' ? (
                    <>
                        <div className="mb-4"><span className={`font-black uppercase text-xs px-4 py-1 rounded-full ${ehAzul ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600'}`}>Equipe {equipeId}</span></div>
                        <div className="text-7xl font-black mb-8 tracking-tighter text-slate-800 animate-in zoom-in">{conta.texto}</div>
                        <form onSubmit={responder}>
                            <input autoFocus type="number" inputMode="numeric" value={input} onChange={e => setInput(e.target.value)} className="w-full text-5xl border-4 rounded-2xl p-4 text-center mb-4 outline-none focus:border-[#46178f] border-slate-100" placeholder="?" />
                            <button className={`w-full ${ehAzul ? 'bg-[#0542b9]' : 'bg-[#c60929]'} text-white font-black py-5 rounded-xl text-2xl shadow-lg active:translate-y-1 transition-all uppercase`}>Enviar</button>
                        </form>
                    </>
                ) : (
                    <div className="py-10 flex flex-col items-center space-y-6">
                        ...
                        <h2 className="text-2xl font-black text-slate-800 uppercase italic">Aguardando...</h2>
                    </div>
                )}
            </div>
        </div>
