...
export default function Equipe01() {
  ...
  return (
    <div className={`h-screen flex flex-col items-center justify-center p-6 text-white transition-all duration-500 ${dados.status === 'jogando' ? 'bg-blue-700' : 'bg-slate-800'}`}>
      ...
      <div className="bg-white text-slate-900 p-10 rounded-[3rem] shadow-2xl w-full max-w-lg text-center border-b-[12px] border-blue-900">
        {dados.status === 'jogando' ? (
          <>
            <p className="text-blue-600 font-black tracking-widest mb-2 uppercase">Equipe 01 - {dados.nivel}</p>
            ...
            <form onSubmit={responder}>
              <input autoFocus type="number" value={input} onChange={e => setInput(e.target.value)}
                className="w-full text-6xl border-4 border-slate-100 bg-slate-50 rounded-3xl p-6 text-center mb-6 focus:border-blue-500 outline-none" />
              <button className="w-full bg-blue-600 text-white font-black py-6 rounded-2xl text-3xl shadow-lg active:translate-y-1 transition-all">ENVIAR</button>
            </form>
          </>
        ) : (
