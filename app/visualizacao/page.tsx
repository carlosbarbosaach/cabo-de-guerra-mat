...
export default function Visualizacao() {
    ...
    useEffect(() => {
        if (dados.status === 'jogando') {
            if (dados.modo === 'cabo') {
                ...
                else if (dados.posicao >= 100) finalizarRodada("EQUIPE 02");
            } else {
    ...
    const resetarTudo = (nivel = dados.nivel) => {
        let opAtual = dados.operacao;
        if (nivel === '1EF' && (opAtual === 'mult' || opAtual === 'div')) opAtual = 'soma';
        set(ref(db, 'partida'), { ...dados, posicao: 0, nivel, tempoRestante: 0, status: 'parado', operacao: opAtual, progressoEquipes: {} });
    };

    const resetarPlacarGeral = () => {
        if (confirm("Deseja realmente ZERAR o placar de pontos e o jogo?")) {
            update(ref(db, 'partida'), { pontosA: 0, pontosB: 0, posicao: 0, status: 'parado', tempoRestante: 0 });
        }
    };

    return (
        <div className="h-screen bg-[#0f172a] flex flex-col items-center justify-between py-6 text-white overflow-hidden bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-slate-950 font-sans">
            {dados.status === 'finalizado' && (
                <div className="absolute inset-0 z-[110] bg-black/95 backdrop-blur-md flex flex-col items-center justify-center">
                    <h2 className="text-8xl font-black mb-10 text-yellow-500 uppercase italic italic">Fim da Rodada!</h2>
                    ...
                    <div className="bg-white/5 backdrop-blur-md p-5 rounded-[2.5rem] border border-white/10 flex flex-col items-center gap-4 shadow-2xl w-[450px]">
                        <div className="flex justify-center gap-1.5 w-full">
                            {['1EF', '5EF', '9EF', '3EM'].map(n => (
                                <button key={n} onClick={() => resetarTudo(n)} className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all ${dados.nivel === n ? 'bg-yellow-500 text-black shadow-lg scale-105' : 'text-slate-500 hover:text-white'}`}>{n}</button>
                            ))}
                        </div>
                        ...
                        <button onClick={resetarPlacarGeral} className="w-full mt-2 bg-red-600/10 hover:bg-red-600 border border-red-500/30 text-red-500 hover:text-white py-2 rounded-lg font-black text-[9px] uppercase transition-all">
                            ⚠️ Zerar Pontuação Geral
                        </button>
                    </div>
                </div>
            ...
            <div className="text-[15rem] font-mono font-black text-white drop-shadow-2xl -mt-10 pointer-events-none select-none">
                {Math.floor(dados.tempoRestante / 60)}:{(dados.tempoRestante % 60).toString().padStart(2, '0')}
            </div>

            <div className="w-full px-20 mb-10 z-10">
                {dados.modo === 'cabo' ? (
                    <div className="w-full h-36 bg-slate-950/60 rounded-[2rem] relative flex overflow-hidden border-4 border-slate-800 shadow-2xl backdrop-blur-sm">
                    ...
                    <div className="grid grid-cols-1 gap-4 p-6 bg-black/30 rounded-[2rem] border border-white/5">
                        {Object.entries(dados.progressoEquipes || {}).map(([id, prog]) => (
                            <div key={id} className="flex items-center gap-8"><span className="font-black text-xl w-32 uppercase italic">Equipe {id}</span><div className="flex-1 h-10 bg-slate-900 rounded-full overflow-hidden border-2 border-slate-800"><div className="h-full bg-gradient-to-r from-purple-600 to-indigo-500 transition-all duration-300" style={{ width: `${prog}%` }} /></div><span className="font-mono font-black text-2xl w-24 text-purple-400 text-right">{prog}%</span></div>
                        ))}
                    </div>
                )}
            </div>
